import datetime

counterparties = [
    {
        "counterparty_id": 1,
        "counterparty_legal_name": "Fahey and Sons",
        "legal_address_id": 15,
        "commercial_contact": "Micheal Toy",
        "delivery_contact": "Mrs. Lucy Runolfsdottir",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 2,
        "counterparty_legal_name": "Leannon, Predovic and Morar",
        "legal_address_id": 28,
        "commercial_contact": "Melba Sanford",
        "delivery_contact": "Jean Hane III",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 3,
        "counterparty_legal_name": "Armstrong Inc",
        "legal_address_id": 2,
        "commercial_contact": "Jane Wiza",
        "delivery_contact": "Myra Kovacek",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 4,
        "counterparty_legal_name": "Kohler Inc",
        "legal_address_id": 29,
        "commercial_contact": "Taylor Haag",
        "delivery_contact": "Alfredo Cassin II",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 5,
        "counterparty_legal_name": "Frami, Yundt and Macejkovic",
        "legal_address_id": 22,
        "commercial_contact": "Homer Mitchell",
        "delivery_contact": "Ivan Balistreri",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 6,
        "counterparty_legal_name": "Mraz LLC",
        "legal_address_id": 23,
        "commercial_contact": "Florence Casper",
        "delivery_contact": "Eva Upton",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 7,
        "counterparty_legal_name": "Padberg, Lueilwitz and Johnson",
        "legal_address_id": 16,
        "commercial_contact": "Ms. Wilma Witting",
        "delivery_contact": "Christy O'Hara",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 8,
        "counterparty_legal_name": "Grant - Lakin",
        "legal_address_id": 16,
        "commercial_contact": "Emily Orn",
        "delivery_contact": "Veronica Fay",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 9,
        "counterparty_legal_name": "Price LLC",
        "legal_address_id": 5,
        "commercial_contact": "Sheryl Langworth",
        "delivery_contact": "Simon Schoen",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 10,
        "counterparty_legal_name": "Bosco - Grant",
        "legal_address_id": 10,
        "commercial_contact": "Ed Halvorson",
        "delivery_contact": "Dewey Kuhic",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 11,
        "counterparty_legal_name": "Corkery - Nienow",
        "legal_address_id": 17,
        "commercial_contact": "Glen Emmerich",
        "delivery_contact": "Laurie Hermiston",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 12,
        "counterparty_legal_name": "Keeling - Emmerich",
        "legal_address_id": 7,
        "commercial_contact": "Marcus Rogahn",
        "delivery_contact": "Rosa Powlowski",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 13,
        "counterparty_legal_name": "Hand, Krajcik and Kuhlman",
        "legal_address_id": 5,
        "commercial_contact": "Sherry Rice",
        "delivery_contact": "Elizabeth Senger",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 14,
        "counterparty_legal_name": "Hintz, Hayes and Will",
        "legal_address_id": 30,
        "commercial_contact": "Ms. Susan Haley",
        "delivery_contact": "Andrea Kassulke PhD",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 15,
        "counterparty_legal_name": "Emmerich, Rohan and Reynolds",
        "legal_address_id": 17,
        "commercial_contact": "Roderick Waters",
        "delivery_contact": "Jake Welch DVM",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 16,
        "counterparty_legal_name": "Hartmann, Franecki and Ratke",
        "legal_address_id": 14,
        "commercial_contact": "Alberta Franecki DDS",
        "delivery_contact": "Tracy Quigley",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 17,
        "counterparty_legal_name": "Kihn Group",
        "legal_address_id": 17,
        "commercial_contact": "Dr. Doris Heidenreich",
        "delivery_contact": "Gregg Beahan",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 18,
        "counterparty_legal_name": "Smith and Sons",
        "legal_address_id": 17,
        "commercial_contact": "Laura Graham",
        "delivery_contact": "Olivia Franey",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 19,
        "counterparty_legal_name": "Ondricka, Conroy and Turcotte",
        "legal_address_id": 18,
        "commercial_contact": "Pearl Kreiger",
        "delivery_contact": "Warren Weimann",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
    {
        "counterparty_id": 20,
        "counterparty_legal_name": "Yost, Watsica and Mann",
        "legal_address_id": 2,
        "commercial_contact": "Sophie Konopelski",
        "delivery_contact": "Janie Doyle",
        "created_at": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
        "last_updated": datetime.datetime(2022, 11, 3, 14, 20, 51, 563000),
    },
]
